...
It can sign transactions locally, and submit the raw transaction data to a remote metrix node. The blockchain data is provided by the metrix expolorer api (which powers https://explorer.metrixcoin.com/), rather than the raw metrixd RPC calls.

This library makes it possible to run DApp without the users having to run a full metrix node.

> This library is extracted from the official [QTUM web wallet](https://github.com/qtumproject/qtum-web-wallet) and modified to utilise the Metrix explorer.

## Install

```
yarn add metrixjs-wallet
```

## Running Tests

The metrixjs-wallet depends on a number of external services. To run the unit tests, you'll need to start the services using [docker-compose](https://docs.docker.com/compose/install/).
...
- Uses satoshi (1e8) as internal units
  - Can represent up to ~90 million MRX accurately.
...
```js
import { networks, generateMnemonic } from "metrixjs-wallet";
...
private key (WIF): pNQKccYYQyGX9G9Qxq2DJev9jHygbZpb2UG7EvUapbtDx5XhkhYE
```
...
The currency unit used is `satoshi`. To convert metrix to satoshi you should multiply the amount you want with `1e8`.

```js
import { networks } from "metrixjs-wallet";
...
async function main() {
  ...
  const toAddr = "mS3ThpDn4HRH9we2hZUdF3F3uR7TTvpZ9v";
  // Sending 100 mrx
  const sendtx = await wallet.send(toAddr, 100 * 1e8, { feeRate: 1000000 });
...
The ABI encoding for the `burnbabyburn()` invokation is `e179b912`. We'll burn 100 metrix, expressed in unit of satoshi.
...
async function main() {
  ...
  const tx = await wallet.contractSend(contractAddress, encodedData, {
    amount: 100 * 1e8 // 100 mrx in satoshi
  });
...
```
public address: mWAnfBnRNhZBqtgSdgHjSfS2D5Jawmafra
```
...
```
{ addrStr: 'mbkJZTKQfcout2joWVmnvUrJUDTg93bhdv',
...
```ts
const toAddress = "mZaTYNEimGLuqnBDpP3KvBKsFs3DbCuwnr";
const amount = 100 * 1e8; // 100 MRX
...
```ts
const tx = await wallet.send(toAddress, amount, {
  // rate is 1000000 satoshi per byte, or  ~10 mrx/KB, as is typical.
  feeRate: 1000000
});
```
...
You can decode the raw transaction using `metrix-cli`:

```
metrix-cli decoderawtransaction 0100000001a09a921ecb38...
  ...
  "vout": [
    {
      ...
      "scriptPubKey": {
        ...
        "type": "pubkeyhash",
        "addresses": [
          "mZaTYNEimGLuqnBDpP3KvBKsFs3DbCuwnr"
        ]
      }
    },
    {
      ...
      "scriptPubKey": {
        ...
        "type": "pubkeyhash",
        "addresses": [
          "mbkJZTKQfcout2joWVmnvUrJUDTg93bhdv"
        ]
      }
    }
  ]
...
1. pubkeyhash 100. This is the amount we want to send.
...
Invoke the `burn()` method, and transfer 10000000000 satoshi to the contract.

- The `burn()` method call ABI encodes to `e179b912`
- The 10000000000 is `msg.value` in contract code.
...
// Invoke a contract's method, and transferring 100 to it.
const tx = await wallet.contractSend(contractAddress, encodedData, {
  amount: 100 * 1e8
});
...
```
metrix-cli decoderawtransaction 0100000001e7b6f2767e8a6...
```
...
1. call 100. This is the amount we want to send to the contract.
