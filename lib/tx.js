...
function buildPubKeyHashTransaction(utxos, keyPair, to, amount, feeRate) {
    ...
    const txb = new bitcoinjs_lib_2.Psbt({ network: keyPair.network });
    ...
    txb.addOutput({ script: buffer_1.Buffer.from(to, 'hex'), value: amount });
    const change = vinSum
        .minus(txfee)
        .minus(amount)
        .toNumber();
    if (change > 0) {
        txb.addOutput({ script: buffer_1.Buffer.from(senderAddress, 'hex'), value: change });
    }
...
function buildCreateContractTransaction(utxos, keyPair, code, feeRate, opts = {}) {
    ...
    const change = totalValue
        ...
        .toNumber();
    if (change > 0) {
        txb.addOutput({ script: buffer_1.Buffer.from(fromAddress, 'hex'), value: change });
    }
    for (let i = 0; i < inputs.length; i++) {
        txb.signInput(i, keyPair);
        txb.validateSignaturesOfInput(i);
